  ...
  useEffect(() => {
    const interval = setInterval(() => {
      orders.forEach(async (order) => {
        ...
        if (result.success) {
          dispatch(
            updateOrderStatus({ orderId: order.orderId, status: newStatus })
          );
        } else {
          console.error(
            `Failed to update order ${order.orderId}: ${result.message}`
          );
        }
      });
                ...
                <div className="flex items-center">
                  ...
                  {order.status === "Out for delivery" && (
                    <span
                      ...
                      onClick={() =>
                        router.push(`/frontend/track?orderId=${order.orderId}`)
                      }
                    >
                      Track live status
                    </span>
